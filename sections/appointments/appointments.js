...
const Appointments = () => {
    ...
    const [appointment, setAppointments] = useState([
        { sNo: 1, appointment: "Consultation", speciality: "Cardiology", date: "2024-06-10" },
        { sNo: 2, appointment: "Follow-up", speciality: "Dermatology", date: "2024-06-11" },
        { sNo: 3, appointment: "Initial Visit", speciality: "Neurology", date: "2024-06-12" },
        { sNo: 4, appointment: "Routine Check", speciality: "Pediatrics", date: "2024-06-13" },
        { sNo: 5, appointment: "Emergency", speciality: "Orthopedics", date: "2024-06-14" },
    ]);
    if (pageLoad) {
        return (
    ...
    return (
        <div>
            <Padding>
                ...
                {
                    activeTab == 1 ? appointment && appointment.length > 0 && (
                        <div>
                            <div className=" border-[#F0F0F0] w-[100%]   border-[1px] rounded-3xl">
                                ...
                                {appointment.map((item, index) => (
                                    <div key={index} className={clsx("flex gap-2 font-circular text-[#000000] py-[1rem]", index % 2 !== 0 ? "bg-[#F9FBFC]" : "bg-white")}>
                                        <div className="w-[20%] items-center flex justify-center">{item.sNo}</div>
                                        <div className="w-[20%] items-center flex">{item.appointment}</div>
                                        <div className="w-[20%] items-center flex justify-center">{item.speciality}</div>
                                        <div className="w-[20%] items-center     flex justify-center">{item.date}</div>
                                        <div className="w-[20%] flex justify-center">
                                            Live
                                        </div>
                                    </div>
                                ...
                                {appointment.map((item, index) => (
                                    <div key={index} className={clsx("flex gap-2 font-circular text-[#000000] py-[1rem]", index % 2 !== 0 ? "bg-[#F9FBFC]" : "bg-white")}>
                                        ...
                                        <div className="w-[20%] items-center flex justify-center">
                                            <Link href={`/dashboard/appointments/singleappointment?id=${item._id}`}>

                                                <Button className={" border-[#E2E4E8] border-[1px] bg-white"} text={"View"} />
                                            </Link>
                                        </div>
                                    </div>
                                ))}
